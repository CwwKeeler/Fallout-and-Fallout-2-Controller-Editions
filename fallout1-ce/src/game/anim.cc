...
{
    for (int i = 0; i < curr_sad; ++i) {
        AnimationSad* move = &sad[i];
...
static int anim_move(Object* obj, int tile, int elev, int a3, int anim, int a5, int animationSequenceIndex)
{
    ...
    sad_entry->continuation = nullptr;
...
static void object_move(int index)
{
    ...
    if ((x > 0 && x <= object->x) || (x < 0 && x >= object->x) || (y > 0 && y <= object->y) || (y < 0 && y >= object->y)) {
        ...
        if (v10 != -1) {
            ...
            if (sad_entry->field_20 == sad_entry->field_1C && !v17 && sad_entry->continuation) {
                int nextAnimation = sad_entry->anim;
                int direction = sad_entry->continuation(object, &nextAnimation);
                if (direction >= 0 && direction < ROTATION_COUNT) {
                    if (nextAnimation != ANIM_RUNNING
                        ...
                        || (object == obj_dude && is_pc_flag(PC_FLAG_SNEAKING) && !perk_level(PERK_SILENT_RUNNING))
                        || !art_exists(art_id(FID_TYPE(object->fid), object->fid & 0xFFF, ANIM_RUNNING, 0, direction + 1))) {
                        nextAnimation = ANIM_WALK;
                    }
                    if (nextAnimation != sad_entry->anim) {
                        int fid = art_id(FID_TYPE(object->fid), object->fid & 0xFFF, nextAnimation, (object->fid & 0xF000) >> 12, object->rotation + 1);
                        CacheEntry* cache;
                        Art* nextArt = art_ptr_lock(fid, &cache);
                        if (nextArt) {
                            int frameCount = art_frame_max_frame(nextArt);
                            art_ptr_unlock(cache);
                            obj_change_fid(object, fid, &temp);
                            rect_min_bound(&dirty, &temp, &dirty);
                            obj_set_frame(object, frameCount > 0 ? object->frame % frameCount : 0, &temp);
                            rect_min_bound(&dirty, &temp, &dirty);
                            sad_entry->anim = nextAnimation;
                            sad_entry->ticksPerFrame = compute_tpf(object, fid);
                        }
                    }
                    sad_entry->rotations[0] = direction;
                    sad_entry->field_20 = 0;
                    sad_entry->field_1C = 1;
                    sad_entry->field_24 = tile_num_in_direction(object->tile, direction, 1);
                }
            }
