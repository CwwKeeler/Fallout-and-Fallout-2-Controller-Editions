...
int tileScrollPixels(int dx, int dy)
{
    ...
    int anchor = tile_center_tile;
    int beforeX, beforeY, afterX, afterY;
    if (tile_coord(anchor, &beforeX, &beforeY, map_elevation) != 0) return -1;
    int tile = tile_num(buf_width / 2 + dx, buf_length / 2 + dy, map_elevation, true);
    // Preserve the engine's map borders and scroll-blocking restrictions.
    if (tile_set_center(tile, 0) != 0) return -1;
    tile_coord(anchor, &afterX, &afterY, map_elevation);
