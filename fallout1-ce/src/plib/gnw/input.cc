...
{
    if (!kb_is_disabled()) GNW_add_input_buffer(character);
}
...
int GNW_input_init(int use_msec_timer)
{
    ...
    gamepadInit("fallout1-ce", gamepadKey, gamepadCharacter, gSdlWindow);
...
int get_input()
{
    ...
    if (!gamepadDispatchText()) process_bk();
...
void GNW95_process_message()
{
    ...
    while (SDL_PollEvent(&e)) {
        ...
        case SDL_KEYDOWN:
        case SDL_KEYUP:
            if (!kb_is_disabled() && !(e.type == SDL_KEYUP && gamepadKeys[e.key.keysym.scancode])) {
